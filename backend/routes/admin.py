...
async def get_admin_overview():
    ...
    total = len(all_users)
    avg_readiness = round(sum(u.get("readiness_score", 0.0) for u in all_users) / total, 1)
    ...
    for u in all_users:
        ...
        current_skills = u.get("current_skills", [])
    ...
    total_modules = sum(u.get("total_modules", 18) or 18 for u in all_users)
    completed_modules = sum(u.get("completed_modules", 0) or 0 for u in all_users)
